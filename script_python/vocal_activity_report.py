# Adapted from https://github.com/jhuus/HawkEars/blob/main/testing/per_second_tester.py.
#
# This calculates statistics on a per-second rather than a per-segment basis.
...
from base_tester import BaseTester, map_codes
...
class PerSecondTester(BaseTester):
    ...
    def get_annotations(self):
        ...
        for i, row in df.iterrows():
            species = row['species']
            if not map_codes is None and species in map_codes:
                species = map_codes[species]
    ...
    def produce_reports(self):
        ...
        df = pd.DataFrame(self.pr_table, columns=['threshold', 'precision', 'recall', 'f1score', 'f5score', 'f25score'])
...
if __name__ == '__main__':
    ...
    parser.add_argument('-t', '--threshold', type=float, default=.8, help=f'Provide detailed reports for this threshold (default = .8)')
